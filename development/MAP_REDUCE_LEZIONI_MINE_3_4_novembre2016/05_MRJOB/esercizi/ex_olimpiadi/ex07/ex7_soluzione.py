...
class MREx_07(MRJob):
    ...
    def lineMapper(self, _, line):
        ...
        try:   # con try except non considero l'intestazione del nome (cioè la prima linea)
            yield splitted_line[-1], (int(splitted_line[6]), int(splitted_line[7]), int(splitted_line[8]))
        except Exception as e:
            # Table Header
            pass #metto pass perchè la prima riga non mi serve e quindi la escludo
...
# scambio country e medals, così posso calcolare il massimo:
